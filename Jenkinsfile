node{
  stage("Scm checkout"){
    git 'https://github.com/DungLL183/docker-tomcat1.git'
  }
  ...
  stage("Deploy docker image to Tomcat server"){
    def dockerRun = 'docker run -p 8080:8080 -d --name my-app-2 dungll183/docker-tomcat:v3'
    sshagent(['dev-server']) {
